...
const ResearchGateIcon = ({ className }: { className?: string }) => (
    <span className={`${className || ''} inline-flex items-center justify-center font-serif text-sm font-bold`}>R</span>
);
...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    const displayEmail = social.email || (
        social.email_user && social.email_domain
            ? `${social.email_user}@${social.email_domain}`
            : undefined
    );
    ...
    return (
        ...
        >
            {/* Profile Image */}
            <div className="max-w-72 mx-auto mb-6">
                <Image
                    src={author.avatar}
                    alt={author.name}
                    width={320}
                    height={420}
                    className="w-full h-auto rounded-2xl object-contain shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105"
                    priority
                />
            </div>
            ...
            <div className="mb-6 relative px-2">
                <div className="mx-auto flex max-w-56 flex-col gap-3 text-sm">
                {socialLinks.map((link) => {
                        ...
                        return (
                            <div key={link.name} className="relative">
                                <button
                                    ...
                                    className={`flex w-full items-center gap-3 rounded-md px-2 py-1.5 text-left transition-colors duration-200 ${isAddressPinned
                                        ? 'text-accent'
                                ...
                                >
                                    {isAddressPinned ? (
                                        <MapPinSolidIcon className="h-5 w-5 flex-shrink-0" />
                                    ) : (
                                        <MapPinIcon className="h-5 w-5 flex-shrink-0" />
                                    )}
                                    <span>{social.location || link.name}</span>
                                </button>
                        ...
                        return (
                            <div key={link.name} className="relative">
                                <button
                                    ...
                                    className={`flex w-full items-center gap-3 rounded-md px-2 py-1.5 text-left transition-colors duration-200 ${isEmailPinned
                                        ? 'text-accent'
                                        : 'text-neutral-600 dark:text-neutral-400 hover:text-accent'
                                        }`}
                                    aria-label={link.name}
                                >
                                    {isEmailPinned ? (
                                        <EnvelopeSolidIcon className="h-5 w-5 flex-shrink-0" />
                                    ) : (
                    ...
                    return (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-3 rounded-md px-2 py-1.5 text-neutral-600 dark:text-neutral-400 hover:text-blue-600 transition-colors duration-200"
                            aria-label={link.name}
                        >
