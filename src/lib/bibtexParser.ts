...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    const metrics = tags.metrics?.split(';').map((item: string) => item.trim()).filter(Boolean) || [];
    ...
    const publication: Publication = {
      ...
      bibtex: reconstructBibTeX(entry, ['selected', 'preview', 'description', 'keywords', 'code', 'pdf', 'pdfurl', 'metrics']),
    };
    ...
    const numberA = extractDisplayNumber(a.title);
    const numberB = extractDisplayNumber(b.title);
...
function extractDisplayNumber(title: string): number | undefined {
...
function getHighlightNames(locale?: string): string[] {
  ...
  const addName = (name?: string) => {
    ...
    if (cleaned) {
      names.add(cleaned);
      const withoutParenthetical = cleaned.replace(/\s*\([^)]*\)/g, '').trim();
      if (withoutParenthetical) {
        names.add(withoutParenthetical);
      }
    }
  };
